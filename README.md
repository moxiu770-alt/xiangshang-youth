...
启动后进入启动海报和登录页。登录后可选择家庭端、学校教师端或校长端；Mock 数据包含 1 所学校、3 个年级、6 个班级、20 名学生、2 个家庭绑定候选、3 个任务、7 项体测和报告/消息/课程建议。Mock 家长绑定示例为：王小明 + `XS-S01`、王小雨 + `XS-S02`；正式环境以学校后台发放的绑定码为准。
...
- 本地草稿、绑定关系、测评状态、班级动态和课程进度会保存，登录失败、恢复失败、空数据和重试状态均由页面处理；本地提交使用独立的 `pendingSync` 状态，兼容旧版本 `submitted` 数据，避免把 Mock 写入误当成学校服务已确认；双端根工作台支持主动刷新，Android 提供刷新按钮与全局错误重试，iOS 支持系统下拉刷新；登录页可打开协议/隐私说明。双端会监听网络状态，在断网时显示“离线模式”提示，明确当前内容可能是本地 Mock/缓存数据。
...
- **逐页状态机**：每个列表、详情、提交、上传和刷新入口都要统一覆盖 `idle / loading / success / empty / error / retry`，并补充防重复提交、超时和取消行为。当前 Mock 主流程、教师通知/班级圈/课程上传、校长统计次级页已补齐主要状态；真实接口接入时仍需把服务端失败和取消映射到同一状态容器。
...
- **质量工程**：iOS XCTest 与首条 SwiftUI UI 冒烟链路已接入；Android JVM/Compose UI Test、关键路径覆盖率、lint/format、依赖和资源检查已接入，`XiangshangYouth_QA_API34` AVD 已实际通过启动、三角色切换、孩子绑定、报告打开/返回仪器链路，视觉快照回归仍需继续补齐。
...
在不计后端的前提下，当前可视为“可演示的一期前端 + Mock 闭环”：视觉/交互内测约 **75%–80%**，一期业务闭环前端约 **65%–70%**，学校试点交付约 **55%–65%**，规模化商业前端约 **40%–50%**。剩余差距主要集中在真实第三方登录/权限渠道、实体设备与平板/横屏矩阵、视觉快照回归、Release 签名与商店审核材料、真实崩溃/埋点服务；Android AVD 仪器测试已通过，但不能替代实体 Android 机型验收，这些也不应被 Mock 或单元测试结果替代。
