...
final class MockRepository: YouthRepository {
    ...
    func report(for student: Student) -> DiagnosisReport {
        ...
        }, abilityTags: ["协调性良好", "敏捷性优秀", "平衡能力待提升"], riskAlerts: scores.contains(where: { $0 < 3 }) ? ["倒退平衡项目偏弱，建议4周后复测"] : ["暂无高风险提示"], trainingAdvice: ["每周进行3次平衡与核心训练，每次15分钟", "练习脚步变向时注意控制重心"], courseSuggestions: [CourseSuggestion(id: "course1", title: "儿童协调性提升课", duration: "12分钟/节", focus: "平衡与敏捷", isPublicBenefit: true), CourseSuggestion(id: "course2", title: "球类基础控球练习", duration: "15分钟/节", focus: "手脚协调", isPublicBenefit: true)], ruleVersion: student.grade.replacingOccurrences(of: "年级", with: "年级运动能力标准 v1.0"))
    }
