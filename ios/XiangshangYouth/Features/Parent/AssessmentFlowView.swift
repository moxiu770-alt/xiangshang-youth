...
struct AssessmentFlowView: View {
    ...
    var body: some View { ScrollView { VStack(spacing: 14) {
        ...
        VStack(spacing: 8) { Image(systemName: category.icon).font(.system(size: 42, weight: .semibold)).foregroundStyle(category.color).frame(width: 78, height: 78).background(category.color.opacity(0.12), in: Circle()); Text("\(state.selectedChild?.name ?? "未选择孩子")的\(category.rawValue)测评").font(.system(size: 18, weight: .bold)); Text(category == .fitness ? "完成身高、体态与学校运动表现记录" : "请根据孩子最近情况完成本次测评") .font(.system(size: 16)).foregroundStyle(.secondary).multilineTextAlignment(.center) }.padding(.top, 14)
        HStack(spacing: 0) { ForEach(steps.indices, id: \.self) { index in VStack(spacing: 5) { Circle().fill(index <= step ? category.color : Color.gray.opacity(0.2)).frame(width: 28, height: 28).overlay { Text("\(index + 1)").font(.system(size: 16, weight: .bold)).foregroundStyle(index <= step ? .white : .secondary) }; Text(steps[index]).font(.system(size: 16)).foregroundStyle(index <= step ? category.color : .secondary).lineLimit(2).multilineTextAlignment(.center).fixedSize(horizontal: false, vertical: true) }.frame(maxWidth: .infinity); if index < steps.count - 1 { Rectangle().fill(index < step ? category.color : Color.gray.opacity(0.2)).frame(height: 2) } } }.padding(.horizontal, AppTheme.pagePadding)
        ReferenceCard { VStack(alignment: .leading, spacing: 10) { Text(completed ? (category == .fitness ? "学校运动表现记录" : "家庭健康记录已保存") : steps[step]).font(.system(size: 15, weight: .bold)); Text(description).font(.system(size: 16)).foregroundStyle(.secondary); if let savedAtText { Label("最后保存：\(savedAtText)", systemImage: "clock").font(.system(size: 16)).foregroundStyle(.secondary) }; if completed { ForEach(steps.indices, id: \.self) { index in VStack(alignment: .leading, spacing: 3) { Text(steps[index]).font(.system(size: 16, weight: .semibold)).foregroundStyle(category.color); Text(archivedEntry(for: index)).font(.system(size: 16)).foregroundStyle(ReferenceColor.navy) }.frame(maxWidth: .infinity, alignment: .leading).padding(9).background(category.color.opacity(0.06), in: RoundedRectangle(cornerRadius: 8)) } } else { if category == .fitness { TextField(inputPlaceholder, text: $answer).textFieldStyle(.roundedBorder).onChange(of: answer) { _, value in state.saveDraft(value, key: draftKey) } } else { StructuredObservationStep(title: questionLabel, options: currentOptions, tint: category.color, allowsMultiple: category == .mental && step == 1, answer: $answer) { value in state.saveDraft(value, key: draftKey) } }; Label("已保存，可稍后继续。", systemImage: "internaldrive").font(.system(size: 16)).foregroundStyle(.secondary) }; if let validationMessage { Text(validationMessage).font(.system(size: 16)).foregroundStyle(.red) }; if category == .fitness && step == 3 { Label("运动发展测试由学校场地端完成，正式 7 项成绩会在上传后同步。", systemImage: "building.2.crop.circle").font(.system(size: 16)).foregroundStyle(ReferenceColor.blue).padding(9).background(ReferenceColor.sky, in: RoundedRectangle(cornerRadius: 8)) }; if completed { Label(category == .fitness ? "学校正式体测报告由场地端生成；本页不生成或替代正式 7 项成绩。" : "这是家长填写并保存在设备的家庭健康记录，不会作为学校正式测评结果同步。", systemImage: category == .fitness ? "building.2.crop.circle" : "lock.shield.fill").font(.system(size: 16)).foregroundStyle(ReferenceColor.blue).padding(9).background(ReferenceColor.sky, in: RoundedRectangle(cornerRadius: 8)) }; if category == .mental && step == 0 { Label("这是家庭感受记录，不会发起第三方授权，也不会生成心理诊断。", systemImage: "heart.text.square.fill").font(.system(size: 16)).foregroundStyle(ReferenceColor.purple).padding(9).background(ReferenceColor.purple.opacity(0.08), in: RoundedRectangle(cornerRadius: 8)) } }.padding(16) }.padding(.horizontal, AppTheme.pagePadding)
        if completed, category != .fitness {
            Button { beginEditing() } label: { Label("重新填写家庭记录", systemImage: "pencil") .font(.system(size: 15, weight: .bold)).frame(maxWidth: .infinity).padding(.vertical, 11).foregroundStyle(category.color).background(category.color.opacity(0.10), in: RoundedRectangle(cornerRadius: 12)) }.buttonStyle(.plain).padding(.horizontal, 16)
        }
        if !completed, category != .fitness, hasArchivedRecord {
            Label("正在编辑新版本；完成保存前，原健康记录仍会保留。", systemImage: "checkmark.shield")
                .font(.system(size: 16)).foregroundStyle(.secondary).padding(.horizontal, 18)
            Button("取消本次编辑，保留原记录") { cancelEditing() }
                .font(.system(size: 16, weight: .semibold)).foregroundStyle(.secondary)
                .buttonStyle(.plain).padding(.horizontal, 16)
        }
        Button { submitCurrentStep() } label: { Text(completed ? "查看健康档案" : step == steps.count - 1 ? "保存健康记录" : "保存并下一步").font(.system(size: 16, weight: .bold)).frame(maxWidth: .infinity).padding(.vertical, 12).foregroundStyle(.white).background(category.color, in: RoundedRectangle(cornerRadius: 12)) }.buttonStyle(.plain).padding(.horizontal, 16)
