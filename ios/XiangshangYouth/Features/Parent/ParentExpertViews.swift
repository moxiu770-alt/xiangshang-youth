...
struct ExpertDetailSheet: View {
    ...
    var body: some View { NavigationStack { VStack(spacing: 14) {
        ...
        if case let .failed(message) = state.workflowState(for: cancelCommandKey) {
            Text(message).font(.system(size: 12)).foregroundStyle(.red).padding(.horizontal, 20)
        }
        if submitted && !failed && !isEditingBooking { VStack(spacing: 9) { Label(state.repository.supportsRemoteAcknowledgement ? "预约已确认。" : "预约已保存，联网后自动同步。", systemImage: "checkmark.circle.fill").font(.system(size: 12)).foregroundStyle(ReferenceColor.green); Text(date).font(.system(size: 12, weight: .semibold)).foregroundStyle(.secondary); Button("修改预约信息") { isEditingBooking = true; state.clearWorkflowState(commandKey) }.buttonStyle(.bordered); Button("取消预约") { Task { if await state.cancelExpertCommand(expertID: expertID, expertName: name) { submitted = false; dismiss() } } }.buttonStyle(.bordered).tint(.red) } } else { VStack(alignment: .leading, spacing: 10) { DatePicker("选择日期", selection: $selectedDate, in: Date()..., displayedComponents: .date).datePickerStyle(.compact).onChange(of: selectedDate) { _, _ in updatePreferredDateDraft() }; Text("可预约时段").font(.system(size: 13, weight: .bold)); if state.usesRemoteDataSource && availableRemoteSlots.isEmpty { Text(state.expertSlotErrors[expertID] ?? "当前暂无可预约时段").font(.system(size: 12)).foregroundStyle(.secondary) }; LazyVGrid(columns: [GridItem(.adaptive(minimum: 120), spacing: 8)], spacing: 8) { ForEach(slotLabels, id: \.id) { slot in Button { selectedSlot = slot.label; selectedRemoteSlot = slot.remote; updatePreferredDateDraft() } label: { Text(slot.label).font(.system(size: 12, weight: .bold)).frame(maxWidth: .infinity).padding(.vertical, 10).foregroundStyle(selectedSlot == slot.label ? .white : ReferenceColor.navy).background(selectedSlot == slot.label ? ReferenceColor.blue : ReferenceColor.sky, in: RoundedRectangle(cornerRadius: 10)) }.buttonStyle(.plain) } }; if !date.isEmpty { Label("已选：\(date)", systemImage: "calendar.badge.clock").font(.system(size: 12)).foregroundStyle(ReferenceColor.blue) }; TextField("咨询说明", text: $note, axis: .vertical).lineLimit(3...5).textFieldStyle(.roundedBorder) }.padding(.horizontal, 20).onChange(of: note) { _, _ in state.saveDraft("\(date)|\(note)", key: draftKey) }; if case let .failed(message) = commandState { Text(message).font(.caption).foregroundStyle(.red) }; Button { Task { let success = isEditingBooking ? await state.rescheduleExpertCommand(name: name, preferredDate: date, note: note, expertID: expertID, serviceID: selectedRemoteSlot?.serviceID, slotID: selectedRemoteSlot?.slotID, scheduledStartAt: selectedRemoteSlot?.scheduledStartAt, scheduledEndAt: selectedRemoteSlot?.scheduledEndAt) : await state.submitExpertCommand(name: name, preferredDate: date, note: note, expertID: expertID, serviceID: selectedRemoteSlot?.serviceID, slotID: selectedRemoteSlot?.slotID, scheduledStartAt: selectedRemoteSlot?.scheduledStartAt, scheduledEndAt: selectedRemoteSlot?.scheduledEndAt); if success { state.clearDraft(draftKey); submitted = true; isEditingBooking = false } } } label: { HStack { if commandState.isSubmitting { ProgressView() }; Text(commandState.isSubmitting ? "正在提交…" : isEditingBooking ? "更新预约" : failed ? "重新提交" : "提交预约") } }.buttonStyle(.borderedProminent).disabled(commandState.isSubmitting || date.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || note.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }
                        ...
                        Button { selected = item } label: {
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 5) {
                                    ...
                                    Text(item.title ?? "健康成长专家").font(.system(size: 12)).foregroundStyle(ReferenceColor.blue)
                                    Text(item.bio ?? "简介、专业领域与服务内容").font(.system(size: 12)).foregroundStyle(.secondary)
                                }
                                Spacer()
                                Image(systemName: "chevron.right").font(.system(size: 12)).foregroundStyle(.secondary)
                            }
                            .foregroundStyle(ReferenceColor.navy).padding(12).background(.white, in: RoundedRectangle(cornerRadius: 12))
                        }
                        .buttonStyle(.plain)
                        .padding(.horizontal, 12)
                    }
                        ...
                        ForEach(visibleAppointmentHistory.prefix(5)) { item in
                            HStack {
                                VStack(alignment: .leading, spacing: 3) {
                                    Text(item.expertName ?? item.expertID ?? "专家预约").font(.system(size: 13, weight: .bold)).foregroundStyle(ReferenceColor.navy)
                                    Text(item.preferredDate ?? item.scheduledStartAt.map { String($0.prefix(16)) } ?? "预约时间待确认").font(.system(size: 12)).foregroundStyle(.secondary)
                                }
                                Spacer()
                                Text(appointmentStatus(item.status)).font(.system(size: 12, weight: .bold)).foregroundStyle(item.status == "cancelled" ? .red : ReferenceColor.blue)
                            }
                            .padding(12)
