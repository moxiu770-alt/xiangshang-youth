...
struct ParentClassCircleDashboard: View {
    ...
    var body: some View {
                ...
                ForEach(visiblePosts) { post in
                    ReferenceCard {
                        VStack(alignment: .leading, spacing: 7) {
                            ...
                            HStack { Button { if likedPostIDs.contains(post.id) { likedPostIDs.remove(post.id) } else { likedPostIDs.insert(post.id) } } label: { Label(likedPostIDs.contains(post.id) ? "已赞" : "点赞", systemImage: likedPostIDs.contains(post.id) ? "hand.thumbsup.fill" : "hand.thumbsup") }.buttonStyle(.plain); Button { commentSubmitted = false; commentDraft = ""; commentPostID = post.id } label: { Label("评论", systemImage: "bubble.left") }.buttonStyle(.plain); Spacer(); ShareLink(item: post.content) { Label("分享", systemImage: "square.and.arrow.up") } }.font(.system(size: 10, weight: .semibold)).foregroundStyle(likedPostIDs.contains(post.id) ? ReferenceColor.blue : .secondary)
                        }
                    }.padding(.horizontal, 12)
                }
                ReferenceCard {
        ...
        .sheet(item: Binding(get: { commentPostID.map(CommentSheetItem.init) }, set: { commentPostID = $0?.id })) { item in
            NavigationStack { VStack(spacing: 14) { if commentSubmitted { Image(systemName: "checkmark.circle.fill").font(.system(size: 44)).foregroundStyle(ReferenceColor.green); Text("评论已发布").font(.headline); Text("班级成员可以看到你的留言。").font(.footnote).foregroundStyle(.secondary); Button("完成") { commentPostID = nil }.buttonStyle(.borderedProminent) } else { Text("给这条动态留言").font(.headline); TextField("说点鼓励的话…", text: $commentDraft, axis: .vertical).textFieldStyle(.roundedBorder); Button("发布评论") { commentSubmitted = true }.buttonStyle(.borderedProminent).disabled(commentDraft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }; Spacer() }.padding(20).navigationTitle("评论").toolbar { ToolbarItem(placement: .topBarTrailing) { Button("取消") { commentPostID = nil } } } }
        }
    }
