...
struct AssessmentFlowView: View {
    ...
    private func submitCurrentStep() { guard completed || !answer.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { validationMessage = "请填写本步骤信息后再继续。"; return }; if completed { if let child = state.selectedChild { router.push(.report(child)) }; return }; if step < steps.count - 1 { withAnimation { step += 1 } } else { state.clearDrafts(prefix: "assessment-\(state.selectedChild?.id ?? "anonymous")-\(category.rawValue)-"); state.clearDraft(progressKey); completed = true; state.completeAssessment(category) } }
}
        ...
        ScrollView {
            VStack(spacing: 10) {
                ...
                if visiblePosts.isEmpty {
                    circleMockPost(author: "李老师", content: "本周运动打卡已开启，欢迎家长分享孩子的练习瞬间。", isTeacher: true, isLiked: $mockPostLiked, commentCount: state.localFeatures.classPostComments.filter { $0.postID == pinnedNoticeID }.count, onComment: { commentSubmitted = false; commentDraft = ""; commentPostID = pinnedNoticeID })
                }
