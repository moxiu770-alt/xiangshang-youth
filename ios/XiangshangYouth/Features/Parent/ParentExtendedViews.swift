...
struct AssessmentFlowView: View {
    ...
    var body: some View { ScrollView { VStack(spacing: 14) {
        ...
        ReferenceCard { VStack(alignment: .leading, spacing: 10) { Text(completed ? "健康记录已保存" : steps[step]).font(.system(size: 15, weight: .bold)); Text(description).font(.system(size: 11)).foregroundStyle(.secondary); if !completed { TextField(inputPlaceholder, text: $answer).textFieldStyle(.roundedBorder).onChange(of: answer) { _, value in state.saveDraft(value, key: draftKey) }; Label("内容会自动保存为草稿，可稍后继续。", systemImage: "internaldrive").font(.system(size: 9)).foregroundStyle(.secondary) }; if let validationMessage { Text(validationMessage).font(.system(size: 10)).foregroundStyle(.red) }; if category == .fitness && step == 3 { Label("运动发展测试由学校场地端完成，正式 7 项成绩会在上传后同步。", systemImage: "building.2.crop.circle").font(.system(size: 10)).foregroundStyle(ReferenceColor.blue).padding(9).background(ReferenceColor.sky, in: RoundedRectangle(cornerRadius: 8)) }; if completed { Label("这不是学校正式体测报告；正式成绩由场地端生成并同步。", systemImage: "checkmark.shield.fill").font(.system(size: 10)).foregroundStyle(ReferenceColor.blue).padding(9).background(ReferenceColor.sky, in: RoundedRectangle(cornerRadius: 8)) }; if category == .mental && step == 0 { Label("将跳转至第三方心理系统授权登录。", systemImage: "checkmark.shield.fill").font(.system(size: 10)).foregroundStyle(ReferenceColor.purple).padding(9).background(ReferenceColor.purple.opacity(0.08), in: RoundedRectangle(cornerRadius: 8)) } }.padding(12) }.padding(.horizontal, 12)
    ...
    }.padding(.bottom, 14) }.background(ReferenceColor.canvas).task {
        completed = state.localFeatures.completedAssessments.contains(completionKey)
    ...
    private var description: String { if completed { return "本次健康记录已保存到本机健康档案；学校正式体测成绩不会由此页面生成。" }; switch category { case .fitness: return ["确认孩子的出生日期、当前身高和体重。", "填写父母身高，系统会计算遗传身高区间。", "按引导上传或记录站立姿态筛查结果。", "选择学校测试完成情况，或暂时跳过等待场地端回传。"][step]; case .vision: return "填写用眼习惯并进行视力筛查结果确认。"; case .oral: return "填写刷牙习惯并确认近期口腔筛查结果。"; case .mental: return "完成授权后，第三方系统将回传测评报告。" } }
