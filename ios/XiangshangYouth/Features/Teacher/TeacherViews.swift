...
struct TeacherMessagesView: View {
    private let items = [("exclamationmark.triangle.fill", "学生预警通知", "王小明体质指标需关注，请及时跟进。", Color.red), ("building.2.fill", "学校通知", "秋季综合测评工作安排已发布。", ReferenceColor.blue), ("calendar.badge.clock", "课程通知", "三年级2班延时课程将在明日 16:30 开始。", ReferenceColor.green), ("gearshape.fill", "系统消息", "测评数据已完成同步。", ReferenceColor.purple)]
    @State private var selectedMessage: TeacherMessageDetail?
    var body: some View { ScrollView { VStack(spacing: 8) { ParentPageNavigation(title: "消息中心", showsBack: true); ForEach(items.indices, id: \.self) { index in let item = items[index]; Button { selectedMessage = TeacherMessageDetail(title: item.1, detail: item.2, time: index == 0 ? "刚刚" : "今天") } label: { HStack(spacing: 10) { Image(systemName: item.0).foregroundStyle(item.3).frame(width: 36, height: 36).background(item.3.opacity(0.10), in: RoundedRectangle(cornerRadius: 10)); VStack(alignment: .leading, spacing: 4) { Text(item.1).font(.system(size: 12, weight: .bold)); Text(item.2).font(.system(size: 9)).foregroundStyle(.secondary) }; Spacer(); Text(index == 0 ? "刚刚" : "今天").font(.system(size: 8)).foregroundStyle(.secondary); Image(systemName: "chevron.right").font(.system(size: 8)).foregroundStyle(.secondary) }.foregroundStyle(ReferenceColor.navy).padding(11).background(.white, in: RoundedRectangle(cornerRadius: 10)) }.buttonStyle(.plain).padding(.horizontal, 12) } }.padding(.bottom, 10) }.background(ReferenceColor.canvas).sheet(item: $selectedMessage) { message in NavigationStack { VStack(alignment: .leading, spacing: 14) { Text(message.title).font(.title3.bold()); Text(message.time).font(.caption).foregroundStyle(.secondary); Divider(); Text(message.detail).font(.body); Spacer() }.padding().navigationTitle("消息详情").toolbar { ToolbarItem(placement: .topBarTrailing) { Button("关闭") { selectedMessage = nil } } } } } }
}
