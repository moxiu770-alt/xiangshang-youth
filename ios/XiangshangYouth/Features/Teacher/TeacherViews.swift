...
struct SportsUploadDashboard: View {
    ...
    var body: some View { ScrollView { VStack(spacing: 10) {
        ...
        if let record = state.localFeatures.courseUploads.first(where: { $0.taskID == taskID }) { Text(record.status == .submitted ? "本机已保存 · 待同步：\(record.attachmentName) · 出勤 \(record.attendanceCount) 人" : "已保存草稿：\(record.notes)").font(.system(size: 9)).foregroundStyle(.secondary).padding(.horizontal, 14) }
    }.padding(.bottom, 10) }
        .background(ReferenceColor.canvas)
        .overlay {
            if let error = state.error, state.data == nil {
